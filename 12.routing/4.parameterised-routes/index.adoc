...
(non parameterised routes do not take precedence over parameterised routes. this is only possible if routes array item position are interchanged  
 const routes: Routes = [
  { path: 'blog/moo', component: MooComponent },
  { path: 'blog/:id', component: BlogComponent },
];
)
If we visited `/blog/moo` we would show `MooComponent`.
